...
export function ResponsiveNavigation({ user, navItems = [] }: ResponsiveNavigationProps) {
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false)
  const [isMobile, setIsMobile] = useState(false)
  const router = useRouter()
  const pathname = usePathname()
  
  // Set default nav items if none provided
  const defaultNavItems = [
    ...
    },
    {
      ...
      active: pathname === "/worker/drives" || pathname.startsWith("/worker/drive/")
    },
    {
      name: "Participants",
      href: "/worker/participants",
      icon: <Users className="h-5 w-5" />,
      active: pathname === "/worker/participants"
    },
    {
      name: "Inventory",
      href: "/worker/inventory",
      icon: <Syringe className="h-5 w-5" />,
      active: pathname === "/worker/inventory"
    },
    {
      name: "Reports",
      href: "/worker/reports",
      icon: <FileText className="h-5 w-5" />,
      active: pathname === "/worker/reports"
    },
    {
      name: "Settings",
                ...
                <Link key={index} href={item.href} className="no-underline">
                  ...
                  >
                    <div className="flex items-center">                      
                        <div className={`mr-3 ${item.active ? 'text-[#8ed500]' : 'text-gray-400'}`}>
                            {item.icon}
                        </div>
  ...
  // For desktop, return the SimpleSidebar (empty div if not logged in)
  return user ? (
    <SimpleSidebar
    ...
    <div></div> // Empty div if not logged in
  );
}